...
def write_obj(obj_name,
    ...
    '''
    if 'obj' not in os.path.splitext(obj_name)[-1]:
        obj_name = obj_name + '.obj'
    mtl_name = obj_name.replace('.obj', '.mtl')
    texture_name = obj_name.replace('.obj', '.jpg')
    ...
    with open(obj_name, 'w') as f:
        ...
        if texture is None:
            ...
            f.write('\nusemtl %s' % material_name)

            # write mtl
            with open(mtl_name, 'w') as f:
                f.write('newmtl %s\n' % material_name)
                s = 'map_Kd {}\n'.format(os.path.basename(texture_name)) # map to image
                # s = 'map_Kd {}\n'.format(os.path.basename(texture_name.replace('.png', '_blend.png')))  # map to image
...
def tensor_vis_landmarks2(images, landmarks, gt_landmarks=None, color = 'g', isScale=True):
    # visualize landmarks
    vis_landmarks = []
    images = images.cpu().numpy()
    predicted_landmarks = landmarks.detach().cpu().numpy()
